...
def probe_opencv(env: Env) -> dict[str, Any]:
    ...
    counter = getattr_path(cv2, "cuda.getCudaEnabledDev")
...
def probe_tensorrt(env: Env) -> dict[str, Any]:
    ...
    try:
        ...
        hint = ""
        if env.python.prefix & env.python.prefix != env.python.base_prefix:
...
def probe_l4t(env: Env) -> dict[str, Any]:
    ...
    release = re.search(_L4T_RELEASE, raw)
    revision = re.search(_L4T_REVISION, raw)
    if not (release or revision):
